const ellipsisPagination = ({
  ...
  const startNumber = currentPage >= displayNumber 
    ? currentPage - Math.floor(displayNumber / 2)
    : 1

  const numberOfSuffixPagination = totalPage - (currentPage + Math.floor(displayNumber / 2))
  
  const prefixNumber = startNumber !== 1 ? [...new Array(prefix)].map((_, i) => i + 1) : []
  const prefixEllipsis = currentPage >= displayNumber;
  const paginate = [...new Array(displayNumber)].map((_, i) => i + startNumber);
  const suffixEllipsis = currentPage <= totalPage - Math.ceil(displayNumber / 2) - suffix;
  const suffixNumber = currentPage <= totalPage - Math.ceil(displayNumber / 2) 
    ? [...new Array(numberOfSuffixPagination <= suffix ? numberOfSuffixPagination : suffix)]
        .map((_, i) => i)
        .reverse()
        .map(i => totalPage - i)
    : []
